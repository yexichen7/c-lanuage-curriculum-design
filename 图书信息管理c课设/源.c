...
void creat_u(userstore p[])
{
	...
	for (i = 0; i < num; i++)
	{
			fscanf(fp, "%s", p[i].data.account);
			fscanf(fp, "%s", p[i].data.password);
			fscanf(fp, "%d", &p[i].data.mark);
	}
}
void up_u(userstore p[])
{
	...
	for (i = 0; i < num; i++)
	{
		if (p[i].data.mark >-1)
		{
			fprintf(fp, "%s ", p[i].data.account);
			fprintf(fp, "%s ", p[i].data.password);
			fprintf(fp, "%d ", p[i].data.mark);
		}
		fprintf(fp,"\n");
	}
}
...
void find(booknode* phead)
{
	...
	while (n)
	{
		...
		case 3:
			...
			while (p)
			{
				if (strcmp(arr, p->data.name) == 0&&strcmp(brr, p->data.writer) == 0)
				{
					printf("书的编号为:");
					printf("%s\n", p->data.id);
					printf("书的名字为:");
...
void add_u(userstore p[])
{
	...
	for (i = n; i < num; i++)
	{
		char arr[20] = { '\0' };
		printf("请输入账号\n");
		scanf("%s", p[i].data.account);
		char brr[20] =  "12345" ;
		strcpy(p[i].data.password, brr);
		printf("输入权限0用户1管理员\n");
		scanf("%d", &p[i].data.mark);
	}
	printf("增加完成\n");
	up_u(p);
}
...
void change_u(userstore p[])
{
	...
	if (s == 1)
	{
		for (i = 0; i < num; i++)
		{
			if (strcmp(p[i].data.account, arr) == 0)
			{
				printf("请输入新密码\n");
...
int find_u(userstore p[],char arr[20])
{
	...
	for (i = 0; i < num; i++)
	{
		if (strcmp(arr, p[i].data.account) == 0)
		{
			 n = i;
		}
	}
...
int main()
{
	...
	while (choose)
	{
		...
		case 1:
			...
			if (m > -1)
			{
				if (secure(pop, m) == 1&&pop[m].data.mark==1)
				{
					add(p);
				}
				else
